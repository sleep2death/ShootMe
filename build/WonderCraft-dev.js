...
(function (PF) {
    var DynamicPotentialsMap = (function () {
        ...
        DynamicPotentialsMap.prototype.debugDraw = function (game, graphics) {
...
(function (PF) {
    var Agent = (function (_super) {
        ...
        function Agent(static_potentials_map, dynamic_potentials_map, subtract_self_potential_from_dynamic_maps_sum) {
            ...
            if (subtract_self_potential_from_dynamic_maps_sum === void 0) { subtract_self_potential_from_dynamic_maps_sum = false; }
            _super.call(this);
            this.subtractSelfPotentialFromDynamicsMapSum = false;
        ...
        Agent.prototype.moveToPosition = function (new_x, new_y, add_trail) {
            if (add_trail === void 0) { add_trail = false; }
        ...
        Agent.DEFAULT_TRAIL_POTENTIAL = 0;
        return Agent;
...
(function (Seed) {
    var SeedRandom = (function () {
        ...
        SeedRandom.prototype.prng = function () {
            return (this.xg.next() >>> 0) / ((1 << 30) * 4);
        };
        return SeedRandom;
    })();
    Seed.SeedRandom = SeedRandom;
    var XorGen = (function () {
...
var Grid = (function () {
    ...
    Grid.prototype.getMap = function () {
        return this.map;
    };
    ...
    Grid.prototype.cookMap = function () {
...
var WonderCraft = (function () {
    function WonderCraft() {
        ...
        this.create = function (game) {
            ...
            _this.staticPFs = game.add.graphics(0, 0);
            ...
            _this.mouseAttractField = new PF.RadialPotentialField();
            _this.mouseAttractField.type = PF.PotentialField.PF_TYPE_ATTRACT;
            _this.dPFMap.addPotentialField(_this.mouseAttractField);
            _this.agent = new PF.Agent();
            _this.agent.addStaticPotentialsMap(_this.sPFMap);
            _this.agent.addDynamicPotentialsMap(_this.dPFMap);
            _this.agent.moveToPosition(20, 20);
            _this.createRandomObstacles();
        };
        this.update = function (game) {
            var p = _this.agent.position;
            if (_this.path_index < _this.path.length - 2) {
                var x = _this.path[_this.path_index];
                var y = _this.path[_this.path_index + 1];
                if (x == p.x && y == p.y) {
                    _this.smoothPath();
                    _this.createPath();
                }
            }
            p = _this.agent.nextPosition();
            _this.agent.moveToPosition(p.x, p.y);
        ...
        this.onMouseDown = function (input) {
            ...
            _this.path = [];
            _this.path_index = 0;
            var dist = _this.grid.planner.search(_this.agent.position.x, _this.agent.position.y, x, y, _this.path);
        };
    ...
    WonderCraft.prototype.smoothPath = function () {
        var index = this.path_index;
        if (index < this.path.length - 5) {
            var head = new PF.Point(this.path[index], this.path[index + 1]);
            var tail = new PF.Point(this.path[index + 4], this.path[index + 5]);
            if (!this.grid.planner.geometry.stabBox(head.x, head.y, tail.x, tail.y)) {
                this.path_index += 2;
            }
        }
    };
    WonderCraft.prototype.createPath = function () {
        ...
        if (this.path.length > 0) {
            ...
            var delta_x = Math.abs(this.agent.position.x - x);
            var delta_y = Math.abs(this.agent.position.y - y);
            var field = new PF.RadialPotentialField();
    ...
    WonderCraft.prototype.createRandomObstacles = function () {
        var xor = new Seed.SeedRandom("Hello, World");
        var len = 80;
        for (var i = 0; i < len; i++) {
            var w = Math.round(xor.prng() * 16);
            var h = Math.round(xor.prng() * 16);
            var x = Math.round(xor.prng() * this.grid.getCol());
            var y = Math.round(xor.prng() * this.grid.getRow());
            var field = new PF.RectangularPotentialField(w, h);
...
},{"l1-path-finder":8,"ndarray":26}],2:[function(require,module,exports){
/*!
...
},{"contour-2d":11,"ndarray":17,"ndarray-ops":12,"ndarray-prefix-sum":16,"robust-orientation":24,"uniq":25}],7:[function(require,module,exports){
'use strict'
...
},{"./vertex":9}],8:[function(require,module,exports){
'use strict'
...
},{"./geometry":6,"./graph":7,"binary-search-bounds":10}],9:[function(require,module,exports){
'use strict'
...
},{"uniq":25}],15:[function(require,module,exports){
"use strict"
...
},{"buffer":2,"iota-array":18}],18:[function(require,module,exports){
"use strict"
...
},{"buffer":2,"dup":17,"iota-array":27}],27:[function(require,module,exports){
